...
object readFileFromResource {
  ...
  def readFromInputResource(input: String): File= {
    var source: Option[InputStream] = None
    var out: Option[OutputStream] = None
    ...
    try {
      source = Option(getClass.getResourceAsStream(input))
      val tempFile = File.createTempFile(String.valueOf(source.hashCode()), ".yaml")
      out = Some(new FileOutputStream(tempFile))
      val buffer = new Array[Byte](2048)
      var length = -1;
      do {
  ...
  def readFromxmlResource(input: String): File= {
    var source: Option[InputStream] = None
    var out: Option[OutputStream] = None
    ...
    try {
      source = Option(getClass.getResourceAsStream(input))
      val tempFile = File.createTempFile(String.valueOf(source.hashCode()), ".xml")
      out = Some(new FileOutputStream(tempFile))
      val buffer = new Array[Byte](2048)
      var length = -1;
      do {
